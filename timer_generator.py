...
def generate_timer_video(duration, output_video, frame_rate=24, alarm_sound="alarm.mp3", alarm_duration=5):
    ...
    try:
        ...
        print("⚠️ No se encontró la fuente especificada. Usando fuente predeterminada.")
    ...
    if missing_countdown_seconds or missing_alarm:
        print(f"Generando {len(missing_countdown_seconds)} frames de countdown y alarma faltantes...")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generador de videos de temporizador.")
    parser.add_argument("-m", "--minutes", type=int, default=0, help="Minutos del temporizador.")
    parser.add_argument("-s", "--seconds", type=int, default=0, help="Segundos del temporizador.")
    parser.add_argument("-a", "--alarm", type=str, default="alarm.mp3", help="Archivo de audio de alarma.")
    parser.add_argument("-o", "--outputfile", type=str, default="timer.mp4", help="Nombre del archivo de salida.")
